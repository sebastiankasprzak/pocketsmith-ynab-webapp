...
import { PocketSmithAccount } from './types';
...
export class PocketSmithClient {
  ...
  async getAccounts(): Promise<PocketSmithAccount[]> {
    try {
      console.log('Fetching PocketSmith accounts...');

      // First get the user ID
      const userResponse = await this.retryRequest(() =>
        this.client.get('/me')
      );
      ...
      // Then fetch accounts using the correct endpoint
  ...
  async validateApiKey(): Promise<boolean> {
    try {
      console.log('Validating PocketSmith API key...');
      ...
      console.log('PocketSmith API key validation successful');
      return true;
    } catch (error: any) {
      console.error('PocketSmith API key validation failed:', error);
      ...
      throw new Error(`Unable to validate PocketSmith API key: ${error.message}`);
    }
  }
}